...
\begin{tikzpicture}[
    box/.style={rectangle, draw=black, thick, rounded corners=2pt,
                ...
                font=\small},
    sidebox/.style={rectangle, draw=gray, thick, rounded corners=2pt,
...
% Identification
\node[box, fill=blue!10] (id) {Records identified through\\database searching\\(n = \textcolor{red}{??})};
\node[box, fill=blue!10, right=2cm of id] (other) {Records from\\reference lists\\(n = \textcolor{red}{??})};

% Merge
\node[box, below=of $(id)!0.5!(other)$] (dedup) {After duplicates removed\\(n = \textcolor{red}{??})};

% Screening
\node[box, below=of dedup] (screen) {Records screened\\(n = \textcolor{red}{??})};
\node[sidebox, right=2.5cm of screen] (excl1) {Excluded by\\title/abstract\\(n = \textcolor{red}{??})};

% Eligibility
\node[box, below=of screen] (elig) {Full-text articles\\assessed for eligibility\\(n = \textcolor{red}{??})};
\node[sidebox, right=2.5cm of elig] (excl2) {Excluded with reasons\\(n = \textcolor{red}{??})\\{\footnotesize No GVHD outcome\\No microbiome data\\Pediatric only (flagged)\\Case report $n<5$}};

% Included
\node[box, fill=green!10, below=1.5cm of elig] (incl) {Studies included in\\scoping review\\(n = \textcolor{red}{??})};

% Tiers
\node[box, fill=green!20, below left=1cm and 0.5cm of incl, text width=3.5cm] (t13) {Tiers 1--3\\Primary studies\\(n = \textcolor{red}{??})};
\node[box, fill=yellow!20, below right=1cm and 0.5cm of incl, text width=3.5cm] (t4) {Tier 4\\Reviews/narratives\\(n = \textcolor{red}{??})};

% Sub-cohort
\node[box, fill=green!40, below=1cm of t13, text width=3.5cm] (sub) {Analytic sub-cohort\\(n = \textcolor{red}{??} studies,\\\textcolor{red}{??} patients)};
...
% Phase labels
\node[rotate=90, anchor=south, font=\footnotesize\bfseries, text=blue!60] at ($(id.west)+(-1.2,0)$) {Identification};
\node[rotate=90, anchor=south, font=\footnotesize\bfseries, text=blue!60] at ($(screen.west)+(-1.2,0)$) {Screening};
\node[rotate=90, anchor=south, font=\footnotesize\bfseries, text=blue!60] at ($(elig.west)+(-1.2,0)$) {Eligibility};
\node[rotate=90, anchor=south, font=\footnotesize\bfseries, text=blue!60] at ($(incl.west)+(-1.2,0)$) {Included};

\end{tikzpicture}
\caption{\textbf{PRISMA flow diagram.} Study identification, screening, and inclusion process. Numbers to be filled after search completion. Studies are categorized into Tiers 1--4. The analytic sub-cohort includes only Tier 1 studies with time-to-GVHD outcomes and publicly available 16S data.}
\label{fig:prisma}
\end{figure}
