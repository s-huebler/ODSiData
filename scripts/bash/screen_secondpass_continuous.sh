...
if claude -p --dangerously-skip-permissions "$MSG" 2> >(tee -a "$STDERR_TMP" >&2); then
        ...
        rc=$?
        echo "[warning] claude exited with $rc on '$next'." >&2
        cat "$STDERR_TMP" >> "$RUN_LOG"
        
        # Check if the error output mentions a rate limit or quota
        if grep -qiE "rate limit|quota|limit|429" "$STDERR_TMP"; then
            echo "[info] Token limit hit. Sleeping for 1 hour before retrying..." >&2
            rm -f "$STDERR_TMP"
            ...
            # Because we didn't add '$next' to PROCESSED_LOG, 
            # the next loop will pull this exact same PDF and try again.
            continue
        else
            # If it's a different kind of error (e.g., file unreadable), stop as usual.
            echo "[error] Non-quota error encountered. Stopping." >&2
            rm -f "$STDERR_TMP"
            exit "$rc"
        fi
    fi
