...
![](images/Inclusion_Exclusion.png){width="674"}
...
```{r}
tree <- qiime2R::read_qza("~/Documents/ODSi/ODSiData/Merging/merged-table.qza")
tree <- tree$data
tree <- data.frame(sample.id = colnames(tree),
                   GreengenesTree = colSums(tree))
```
...
```{r}
# --- Build your main plot (same as before, but remove bottom legend) ---
read_attrition_option5 <- ggplot(df_long, aes(y = sample.id, x = Reads, fill = Pipeline_Step)) +
  geom_bar(stat = "identity", position = "identity") +
  scale_fill_manual(values = my_colors3) +
  facet_grid2(
    study ~ ., 
...
# --- Create the grouping label strip as a separate grob ---
# Adjust x positions (0-1 scale) to align with legend color swatches
# These will need tweaking based on your final legend width

make_bracket_grob <- function() {
  grob_list <- grobTree(
    
    # ── DADA2 bracket (covers: Input, Filtered, Denoised, Merged, Non-chimeric) ──
    linesGrob(x = unit(c(0.22, 0.22, 0.68, 0.68), "npc"),
              y = unit(c(0.85, 0.5, 0.5, 0.85), "npc"),
              gp = gpar(col = "black", lwd = 1)),
    textGrob("DADA2", x = unit(0.42, "npc"), y = unit(0.1, "npc"),  # centered in bracket
             gp = gpar(fontsize = 9)),
    
    # ── BLAST bracket (covers: BLAST only) ──
    linesGrob(x = unit(c(0.69, 0.69, 0.75, 0.75), "npc"),
              y = unit(c(0.85, 0.5, 0.5, 0.85), "npc"),
              gp = gpar(col = "black", lwd = 1)),
    textGrob("BLAST", x = unit(0.705, "npc"), y = unit(0.1, "npc"),  # centered in bracket
             gp = gpar(fontsize = 9)),
    
    # ── Greengenes2 bracket (covers: OTU Clustered, Reference Tree) ──
    linesGrob(x = unit(c(0.76, 0.76, 1, 1), "npc"),
              y = unit(c(0.85, 0.5, 0.5, 0.85), "npc"),
              gp = gpar(col = "black", lwd = 1)),
    textGrob("Greengenes2", x = unit(0.885, "npc"), y = unit(0.1, "npc"),  # centered in bracket
             gp = gpar(fontsize = 9))
  )
  return(grob_list)
}

# --- Combine plot + bracket strip ---
final_plot <- plot_grid(
  read_attrition_option5,
  make_bracket_grob(),
  ncol = 1,
  rel_heights = c(1, 0.08)  # small sliver for the bracket row
)
...
```{r}
reference_grid <- ggplot() +
  theme_void() +
  geom_vline(xintercept = seq(0, 1, by = 0.01), color = "red", linewidth = 0.4) +
  geom_text(aes(x = seq(0, 1, by = 0.1), y = 0.5, 
                label = seq(0, 1, by = 0.1)), 
...
final_plot <- plot_grid(
  read_attrition_option5,
  make_bracket_grob(),
  reference_grid,  # <-- add this temporarily
  ncol = 1,
  rel_heights = c(1, 0.06, 0.06)
)
print(final_plot)
```
